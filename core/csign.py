...
from core.viewsm import VTTD,STTD
from core.authm import User
from core.extension import mysql

bp = Blueprint('cgen', __name__)

QR = "static/images"
UPLOAD_FOLDER_FILE = "static/upload/file"
@bp.route('/cgen', methods=['GET', 'POST'])
def cgen(docdata, page, region, x1,y1,x2,y2,id_t,email,id_p,nfile,skn,vkn,nsed,nsig,skc):
    dtsig = STTD.get_by_id_sign(mysql,id_t)    
    dtu   = User.get_by_id(mysql,id_p)
    nama  = dtu['nama']
   ...
   #fp.save(os.path.join(UPLOAD_FOLDER_FILE, fname))     
    ...
    if nnum == dtsig['nsig']:
      skn ="Proses Selesai"
      skc ="Proses Selesai"
    STTD.update(mysql, id_t, namafile5,hashfile2,skn,skc, vkn, nsig, nnum, uid1, tsig1, uid2, tsig2, uid3, tsig3, uid4, tsig4)
    os.close(_fr)
    session['mid'] = id_t
    return id_t