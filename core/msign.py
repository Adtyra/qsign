...
bp = Blueprint('mgen', __name__)

QR = "static/images"
UPLOAD_FOLDER_FILE = "static/upload/file"
@bp.route('/mgen', methods=['GET', 'POST'])
def mgen(docdata, page, region, x1,y1,x2,y2,id_t,email,nama,nfile,skn,vkn,nsed,nsig,skc):
   ...
   # icon_img = os.path.join(Logo_link, "icon.png")
    ...
    namafile4 = namafile3+'_created_by_'+nama
   ...
   #fp.save(os.path.join(UPLOAD_FOLDER_FILE, fname))     
    ...
    STTD.create(mysql, id_t, namafile5,hashfile2, skn, skc, vkn, nsig, nnum, uid1, tsig1, uid2, tsig2, uid3, tsig3, uid4, tsig4)
    os.close(_fr)
    session['mid'] = id_t
    return id_t